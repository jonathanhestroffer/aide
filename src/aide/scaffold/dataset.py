...
class ProceduralShapesDataset(Dataset):
    ...
    def __init__(self, num_samples=500, img_size=32, noise_level=0.15):
    ...
    def _draw_shape(self, canvas, shape_type, center, size, color):
        cy, cx = center
...
def create_procedural_shapes_artifacts(artifact_dir: str | Path, *, seed: int = 42) -> Path:
    """Create or reuse a procedural shapes dataset under ``<artifact_dir>/procedural_shapes``."""
    output_dir = Path(artifact_dir).expanduser().resolve() / "procedural_shapes"
    ...
    train_source = ProceduralShapesDataset(num_samples=10000)
    val_source = ProceduralShapesDataset(num_samples=2000)
    test_source = ProceduralShapesDataset(num_samples=2000)

    # Placeholder for actual dataset creation logic
    # For demonstration, we will create empty files to simulate the dataset
    train_path = output_dir / "train.pt"
    val_path = output_dir / "val.pt"
    test_path = output_dir / "test.pt"

    torch.save(train_source, train_path)
    torch.save(val_source, val_path)
    torch.save(test_source, test_path)
        ...
        json.dumps(
            {
                ...
                "meta": {
                    ...
                    "num_train": len(train_source),
                    "num_val": len(val_source),
                    "num_test": len(test_source),
                    "num_classes": train_source.num_classes,
                    "seed": seed,
                },
            },
            indent=2,
